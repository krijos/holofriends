...
function HoloFriendsFuncs_WhoCheckPlayer(name0, quiet, who)
	...
	if ( quiet and not who ) then
		...
		C_ChatInfo.SendAddonMessage("amcetest", "", "WHISPER", name);
		C_ChatInfo.SendAddonMessage("amcetest", "", "WHISPER", "amcetestplayer");
		return;
	end

	-- reset HF_LibWhoCount in case of a missed libwho-event
	-- if who-requests executed nearly the same time
	if ( HF_LibWhoCount < 0 ) then HF_LibWhoCount = 0; end
