...
HF_FAQStrings[idx] = HF_FAQStrings[idx].."\n- "..HOLOFRIENDS_LISTFEATURES25;
HF_FAQStrings[idx] = HF_FAQStrings[idx].."\n- "..HOLOFRIENDS_LISTFEATURES26;
HF_FAQStrings[idx] = HF_FAQStrings[idx].."\n- "..HOLOFRIENDS_LISTFEATURES27;
idx=idx+1;
...
HF_FAQStrings[idx] = HF_FAQStrings[idx].."\n- "..HOLOFRIENDS_LISTFEATURES45;
HF_FAQStrings[idx] = HF_FAQStrings[idx].."\n- "..HOLOFRIENDS_LISTFEATURES46;
idx=idx+1;
...
function HoloFriendsOptions_Button_OnClick(self)
	...
	if ( index ) then
		...
		if ( self:GetChecked() ) then
			...
			local tmpid = HF_TempOptions[index];
			...
			-- change count of FL or IL buttons and change button id's
			if ( fname ) then
				HF_nButtons = HF_nButtons - 1;
				for index = 1, table.getn(HF_OptionNames) do
					if (strsub(HF_OptionNames[index],1,8) == "ButtonFL") then
						if ( HF_TempOptions[index] and HF_TempOptions[index] > tmpid ) then
							HF_TempOptions[index] = HF_TempOptions[index] - 1;
						end
					end
				end
			end
			if ( iname ) then
				...
				for index = 1, table.getn(HF_OptionNames) do
					if (strsub(HF_OptionNames[index],1,8) == "ButtonIL") then
						if ( HF_TempOptions[index] and HF_TempOptions[index] > tmpid ) then
							HF_TempOptions[index] = HF_TempOptions[index] - 1;
						end
					end
