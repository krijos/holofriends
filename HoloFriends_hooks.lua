...
-- this line is modified related to original function
	local namelist = GetAutoCompleteResults(text, 1, utf8Position, unpack(editBox.autoCompleteParams));
-- this line is added to the original function
...
function HoloFriends_AutoComplete_Update(parent, text, cursorPosition)
	...
	if ( cursorPosition <= strlen(text) ) then
		...
		possibilities = GetAutoCompleteResults(text, AUTOCOMPLETE_MAX_BUTTONS+1, cursorPosition, unpack(parent.autoCompleteParams));
...
function HoloFriends_UnitPopup_OnClick(self)
	...
	local id = dropdownFrame.userData.id;
...
function HoloFriends_UnitPopup_OnUpdate(elapsed)
	...
	if ( not UnitPopup_HasVisibleMenu() ) then return; end
